...
- **The snapshot JSON shape is the contract between backend and frontend.** Four parts — `global` (KPIs), `protocols` (donut), `flows` (table rows), and `alerts` (anomaly panel). First three produced in `FlowTable.tick()`, `alerts` attached in `main._aggregation_loop`. Consumed in `app.js` `render()`/`renderFlows()`/`renderAlerts()`/`openDrawer()`. Change a field in one place → update the other.
